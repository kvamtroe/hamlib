 ...
 *
 *	$Id: rig.c,v 1.58.2.3 2003-02-25 06:01:14 dedmons Exp $
 *
...
RIG *rig_init(rig_model_t rig_model)
{
	...
	rs->vfo_list = 0;
	for (i = 0;
	     i < FRQRANGESIZ && !RIG_IS_FRNG_END(rs->rx_range_list[i]);
	     i++) {
		rs->vfo_list |= rs->rx_range_list[i].vfo;
	}
	for (i = 0;
	     i < FRQRANGESIZ && !RIG_IS_FRNG_END(rs->tx_range_list[i]);
	     i++) {
		rs->vfo_list |= rs->tx_range_list[i].vfo;
	}

	memcpy(rs->preamp, caps->preamp, sizeof(int) * MAXDBLSTSIZ);
	memcpy(rs->attenuator, caps->attenuator,
	...
	memcpy(rs->chan_list, caps->chan_list,
...
pbwidth_t rig_passband_narrow(RIG * rig, rmode_t mode)
{
	...
	for (i = 0; i < FLTLSTSIZ - 1 && rs->filters[i].modes; i++) {
		if (rs->filters[i].modes & mode) {
			...
			for (i++; i < FLTLSTSIZ && rs->filters[i].modes;
			     i++) {
				if ((rs->filters[i].modes & mode)
...
int rig_set_split_mode(RIG * rig, vfo_t vfo, rmode_t tx_mode,
		       pbwidth_t tx_width)
{
	const struct rig_caps *caps;
	int retcode;
 ...
 */
int rig_get_split_mode(RIG * rig, vfo_t vfo, rmode_t * tx_mode,
		       pbwidth_t * tx_width)
{
	const struct rig_caps *caps;
	int retcode;
 ...
 * \sa rig_mW2power()
 */
int rig_power2mW(RIG * rig, unsigned int *mwpower, float power,
		 freq_t freq, rmode_t mode)
{
	...
	if (rig->caps->power2mW != NULL)
		return rig->caps->power2mW(rig, mwpower, power, freq,
					   mode);
 ...
 * \sa rig_power2mW()
 */
int rig_mW2power(RIG * rig, float *power, unsigned int mwpower,
		 freq_t freq, rmode_t mode)
{
	const freq_range_t *txrange;
	...
	if (rig->caps->mW2power != NULL)
		return rig->caps->mW2power(rig, power, mwpower, freq,
					   mode);
...
int rig_set_tone(RIG * rig, vfo_t vfo, tone_t tone)
{
	const struct rig_caps *caps;
	int retcode;
	vfo_t curr_vfo;
	...
	if (caps->set_tone == NULL)
		return -RIG_ENAVAIL;

	if ((caps->targetable_vfo & RIG_TARGETABLE_ALL) ||
	    vfo == RIG_VFO_CURR || vfo == rig->state.current_vfo)
		return caps->set_tone(rig, vfo, tone);
	...
	retcode = caps->set_tone(rig, vfo, tone);
	caps->set_vfo(rig, curr_vfo);
	return retcode;
}
...
int rig_get_tone(RIG * rig, vfo_t vfo, tone_t * tone)
{
	const struct rig_caps *caps;
	int retcode;
	vfo_t curr_vfo;
	...
	if (caps->get_tone == NULL)
		return -RIG_ENAVAIL;

	if ((caps->targetable_vfo & RIG_TARGETABLE_ALL) ||
	    vfo == RIG_VFO_CURR || vfo == rig->state.current_vfo)
		return caps->get_tone(rig, vfo, tone);
	...
	retcode = caps->get_tone(rig, vfo, tone);
	caps->set_vfo(rig, curr_vfo);
	return retcode;
}
 ...
 */
static int generic_retr_extl(RIG * rig, const struct confparams *cfp,
			     rig_ptr_t ptr)
{
...
int generic_save_channel(RIG * rig, channel_t * chan)
{
	...
	if (chan->split != RIG_SPLIT_OFF) {
		...
		rig_get_split_mode(rig, RIG_VFO_CURR, &chan->tx_mode,
				   &chan->tx_width);
	} else {
	...
	for (i = 0; i < RIG_SETTING_MAX; i++) {
		int fstatus;
		if (rig_get_func
		    (rig, RIG_VFO_CURR, rig_idx2setting(i),
		     &fstatus) == RIG_OK) chan->funcs |=
			    fstatus ? rig_idx2setting(i) : 0;
	}
				  ...
				  freq_t freq, rmode_t mode)
{
	...
	for (i = 0; i < FRQRANGESIZ; i++) {
		...
		if (freq >= range_list[i].start
		    && freq <= range_list[i].end
