 ...
 *
 * ts2kctl.c - (C) Dale E. Edmons (minor changes uniq to ts2k)
 *
...
struct test_table {
	unsigned char cmd;
	const char *name;
	int (*rig_routine) (RIG *, int, const struct test_table *,
...
declare_proto_rig(set_tone);
declare_proto_rig(get_tone);
...
struct test_table test_list[] = {
	...
	{'M', "set_mode", set_mode, ARG_IN, "Mode\t"},
	{'m', "get_mode", get_mode, ARG_OUT, "Mode"},
	...
	{'C', "set_tone", set_tone, ARG_IN, "Tone\t"},
	{'c', "get_tone", get_tone, ARG_OUT, "Tone"},
	{'D', "set_dcs", set_dcs, ARG_IN, "DCS code\t"},
	{'d', "get_dcs", get_dcs, ARG_OUT, "DCS code"},
	{'I', "set_split_freq", set_split_freq, ARG_IN, "Tx frequency"},
	{'i', "get_split_freq", get_split_freq, ARG_OUT, "Tx frequency"},
	{'X', "set_split_mode", set_split_mode, ARG_IN, "Mode\t"},
	{'x', "get_split_mode", get_split_mode, ARG_OUT, "Mode"},
	...
	{'u', "get_func", get_func, ARG_IN1 | ARG_OUT2, "Func",
	 "Func status"},
	...
	{'H', "set_channel", set_channel, ARG_IN, "MemChan#", "VFO" },
	{'h', "get_channel", get_channel, ARG_IN|ARG_OUT1|ARG_OUT2, "MemChan#", "VFO" },
...
int main(int argc, char *argv[])
{
	...
	while (1) {
		...
		case 'C':
			...
			if (*conf_parms != '\0')
				strcat(conf_parms, ",");
			strncat(conf_parms, optarg,
				MAXCONFLEN - strlen(conf_parms));
			break;
		case 'v':
			verbose++;
			break;
	...
	if (rig_file)
		strncpy(my_rig->state.rigport.pathname, rig_file,
			FILPATHLEN);
	...
	if (ptt_file)
		strncpy(my_rig->state.pttport.pathname, ptt_file,
			FILPATHLEN);
	if (dcd_file)
		strncpy(my_rig->state.dcdport.pathname, dcd_file,
			FILPATHLEN);
	/* FIXME: bound checking and port type == serial */
	if (serial_rate != 0)
		my_rig->state.rigport.parm.serial.rate = serial_rate;
	if (civaddr)
		rig_set_conf(my_rig, rig_token_lookup(my_rig, "civaddr"),
			     civaddr);
	 ...
	 */
	if (show_conf) {
		rig_token_foreach(my_rig, print_conf_list,
				  (rig_ptr_t) my_rig);
	}
	...
	if (verbose > 0)
		printf("Opened rig model %d, '%s'\n",
		       my_rig->caps->rig_model, my_rig->caps->model_name);
...
static int print_conf_list(const struct confparams *cfp, rig_ptr_t data)
{
	...
	case RIG_CONF_COMBO:
		...
		for (i = 1; i < (RIG_COMBO_MAX && cfp->u.c.combostr[i]);
		     i++) {
...
{
	printf("%d\t%-14s%-16s%s\n", caps->rig_model,
	       caps->mfg_name, caps->model_name, caps->version);
	return 1;		/* !=0, we want them all ! */
}
...
/*
 * static int (f)(RIG *rig, int interactive,\
...
declare_proto_rig(set_tone)
{
	...
	sscanf(arg1, "%d", &tone);
	return rig_set_tone(rig, RIG_VFO_CURR, tone);
}
...
declare_proto_rig(get_tone)
{
	...
	status = rig_get_tone(rig, RIG_VFO_CURR, &tone);
	if (status != RIG_OK)
		return status;
	if (interactive)
		printf("%s: ", cmd->arg1);
...
void dump_chan(RIG * rig, channel_t * chan)
{
	...
	printf("Tone: %1.1fHz, ", ((float)chan->tone) / 10.0);
	printf("ToneSql: %01iHz, ", chan->tone_sql);
	printf("DCS: %i, ", chan->dcs_code);
	printf("DCSsql: %i\n", chan->dcs_sql);
